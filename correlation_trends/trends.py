...
def trend_with_significance(t: ArrayLike, x: ArrayLike, dt: float) -> TrendResult:
    ...
    se = float(np.sqrt(mse / sst))

    n_eff = effective_dof(resid, dt)
    n_eff = max(n_eff, 3.0)

    se_eff = se * float(np.sqrt(n / n_eff))

    t_naive = slope / se
    t_eff   = slope / se_eff

    p_naive = float(2 * stats.t.sf(abs(t_naive), df=n - 2))
    p_eff   = float(2 * stats.t.sf(abs(t_eff),   df=max(n_eff - 2, 1)))
